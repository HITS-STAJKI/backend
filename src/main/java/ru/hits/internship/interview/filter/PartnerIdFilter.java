...
@Component("interviewFilterPartnerId")
public class PartnerIdFilter implements Filter<InterviewEntity, InterviewFilter> {

    @Override
    public boolean isApplicable(InterviewFilter filter) {
        return filter.getPartnerId() != null;
    }
    ...
    public Predicate toPredicate(Root<InterviewEntity> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder, InterviewFilter filter) {
        return criteriaBuilder.equal(root.get("company").get("id"), filter.getPartnerId());
    }
}
